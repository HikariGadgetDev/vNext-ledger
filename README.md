...
## 🔧 JSON エクスポートと再検討フロー（活用フェーズ）

vNext Ledger は  
「記録して終わり」ではありません。

記録された設計判断は、  
**JSON として切り出され、再検討の俎上に戻ることを前提に設計されています。**
...
以下の API により、  
台帳の状態は **機械可読な一次資料**として取得できます。

```bash
# 台帳の全ノート
curl http://127.0.0.1:8000/export/notes

# scan 実行履歴
curl http://127.0.0.1:8000/export/scan_history

# メトリクス（diff / full 実行状況・件数）
curl http://127.0.0.1:8000/export/metrics
```
...
- どの判断が どれだけ溜まっているか
- diff scan だけ回っていて 収束が止まっていないか
- evidence（観測点）が 増えているか／止まっているか

を、**人ではなく構造で確認**できます。

---

### CI / 定期レビュー向け：JSON ファイル生成

vNext Ledger には、  
JSON を日付ディレクトリに書き出すための **薄いラッパースクリプト**が用意されています。
...
- 定期レビュー
- 設計振り返り
- 学部や院での研究報告、PM向け資料、助成金関連資料
- 「今、何を抱えているか」の自己説明

といった用途に **そのまま使える一次資料**です。
...
3. JSON として 外に切り出す
4. 人間が再検討する
5. 決着したら `DONE(vNext)` に変える
6. full scan で 世界を収束させる
...
vNext Ledger は **UI を持ちません**。

- ダッシュボード
- 優先度操作
- フロー管理

これらを最初から提供しないのは **意図的**です。
...
を持った極めて実務的な道具です。  
人が入れ替わり、時間が経っても、**設計の意図だけは失われない**ことを目指しています。
